...
export function checkInstallDocs(root = repoRoot) {
  ...
  for (const host of ["Codex", "Claude Code", "OpenCode", "Grok Build"]) {
  ...
  return Object.freeze({ files: AUTHORITY_FILES.length, hosts: 4, slashHosts: 3 });
}
...
if (invoked) {
  try {
    ...
    process.stdout.write(
      ...
      + `codex=skill-first slash_hosts=${result.slashHosts} tiers=15/30/60 legacy_patterns=0\n`,
    );
  } catch (error) {
