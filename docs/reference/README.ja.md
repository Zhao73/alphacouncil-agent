...
<b>質問 → 出典付き根拠 → 固定した手法スタンス → 強気・弱気の反論 → PM 判断 + 保存可能な監査記録</b>

<sub><i><a href="../../assets/demo.mp4">過去版 UI の録画</a> · <a href="../examples/final_report.SOX.zh.md">過去版レポート成果物</a>（SOX、中国語）。いずれも現在の 26 席候補より前のもので、現在の所要時間、手法忠実度、データ精度、4 ホスト E2E を証明しません。</i></sub>
...
| 🔍 **監査可能、欠落を隠さない** | 重要な主張は source ID に紐づけます。システムは誤る可能性があるため、欠落・出典・診断を残して確認できるようにします。 |
| ⏱️ **マルチ期間の判定** | 買い/中立/売りに加え、1〜4週・3〜6か月・12か月の見通しを個別に提示。 |
| 🔑 **キー要件を明示** | 最初の米国/台湾/市場データ確認はデータベンダーキー不要。DART・EDINET は任意の無料キーを使い、評議会ワーカーには認証済みホスト/モデルが必要です。MIT ライセンス。 |
...
固定 52 項目を一つずつ説明し、all ではさらに 3 packet を追加します。
サーバーは各メソッド席、各 Bull/Bear、PM の直前に完全資料を再読込・再ハッシュし、全 decision claim、
参照 source、52 項目の結果、packet ごとの結論・data・metric・gap を含む限定的で完全な decision projection を渡します。
26 の各メソッド席が数 MB の生の取得記録や大容量時系列を再度開くことはありません。全 worker は同じ SHA-256 を確認し、重大な欠落があれば判断を停止します。
...
| --- | --- | --- | --- | --- |
| `fast` | 15 分（設定約 14.3 分） | 未検証 | 4 分 | 85 秒 |
...
最初の米国/台湾/市場データ確認はデータベンダーキー不要です。任意の DART・EDINET
データ源には文書記載の無料キー、評議会調査には認証済みホスト/モデルが必要です。
