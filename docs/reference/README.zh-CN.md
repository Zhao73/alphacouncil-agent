...
全部所选方法席必须读取完整档案；服务端在每个下游席位启动前重读并校验全文哈希，再向三轮多空和 PM
注入包含全部决策论断、引用来源、52 项覆盖结果与缺口的有界决策投影。所有席位确认同一哈希；关键资料
缺失时停止决策，原始采集记录和大体量时间序列保留在完整审计档案中，不在每轮重复处理。
...
| --- | --- | --- | --- | --- |
| `fast` 快速 | 15 分钟（配置约 13.5 分钟） | 尚未验证 | 4 分钟 | 85 秒 |
