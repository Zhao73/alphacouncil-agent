...
<b>问题 → 带来源证据 → 冻结方法立场 → 多空质询 → PM 决策 + 可保存审计链</b>

<sub><i><a href="../../assets/demo.mp4">历史界面录屏</a> · <a href="../examples/final_report.SOX.zh.md">历史报告工件</a>（SOX，中文）。两者都早于当前 26 席候选版，不证明当前耗时、方法忠实度、数据准确性或四宿主 E2E。</i></sub>
...
| ⏱️ **完整 headless 有硬时限，且有三档深度** | 开场分别选择方法席、分析席范围（核心 8 / 全部 11）和 fast / normal / slow（持久化上限 15 / 30 / 60 分钟）。界面明确显示完整完成实测尚未验证，不把配置预算冒充预计耗时。 |
| 🔍 **可审计，缺口不隐藏** | 重要结论必须映射到 source ID；缺失数据写进「数据缺口」章节。系统仍可能出错，因此保留来源和诊断供复核。 |
| ⏱️ **多周期结论** | 买入/持有/卖出,外加独立的 1-4 周、3-6 月、12 月判断。 |
| 🔑 **34 个工具，密钥边界明确，零运行时依赖** | 首次美国/台湾/市场数据检查不需要数据商密钥；DART、EDINET 覆盖使用可选免费密钥；议会工作单元需要已认证的宿主/模型。MIT 开源。 |
...
确定性测试和 fixture 会逐席覆盖 policy、类型化事实与弃权路径，但这不等于真实的 26 席终态运行，
也不能证明方法忠实度或有用性。当前正式计数仍是：规范评估运行 0/8，真实宿主 E2E 0/4。
...
精确的 ETF/指数与 full/quick 边界见 [v1.0.0 发布合同](../releases/v1.0.0.md)，`quick_v1` 与 `full_v2` 的报告差异见
[报告合同](../report-contract.md)。
...
**[docs/INSTALL.md](../INSTALL.md)**。**Windows 用户**见 [Windows 小节](../INSTALL.md#windows)。
...
子代理路径,见 [docs/INSTALL.md](../INSTALL.md)。
...
会显示系统价格（或明确的行情缺口）、本轮全部 8 或 11 个分析师的状态/摘要，以及每个所选方法席冻结的立场
...
完整公司决策还会写出 `company_dossier.json`：8 个核心证据包共同覆盖固定 52 项公司资料清单；
选择“全部分析席”时，再冻结 3 个补充证据包。每项都明确为已覆盖、实际检索后不可得或确实不适用，并保留来源/主张链和统一 SHA-256。
服务端在每个方法席、三轮多空和 PM 启动前重读并校验完整档案，再注入包含全部决策论断、引用来源、
52 项覆盖结果、逐包结论与数据、指标及缺口的有界完整决策投影。26 个方法席不再分别重开多 MB 的原始
采集记录和大体量时间序列。所有席位确认同一哈希并逐包回执；关键资料缺失时停止决策。
...
| 输入 | 跑什么 | 额度消耗 |
|---|---|---|
| `/alpha <ticker>` | 先问跑多深（三档，显示持久化上限与“完整完成尚未验证”），再逐人展示全部大师，确认后运行 full | 每个达成立场的 v3 席为确定性立场 + 一个独立 voice worker；弃权席直接发布 |
...
`begin_council_selection` 会返回每档的持久化硬上限、配置分段预算和
`observed_completion_status`。配置预算不是实测完成时间。

| 档位 | 持久化硬上限 | 完整完成实测 | 每证据席 | 每轮辩论每侧 |
| --- | --- | --- | --- | --- |
| `fast` 快速 | 15 分钟（配置约 14.3 分钟） | 尚未验证 | 4 分钟 | 85 秒 |
...
`incomplete`），不保证成功完成；取得预注册真实终态证据前，不发布任何档位的完成时间。
...
本轮确认的 8 或 11 个证据席同波并行。证据门禁通过后，每个所选物理 v3 方法先运行确定性政策并冻结立场；
已形成评分的方法席会获得一个只属于该 stable ID 的独立 voice worker（可以解释立场，但不能改
立场或补造 typed fact）；带有冻结哈希的 `out_of_scope` 席位保留确定性解释，不再调用解释模型。
每个方法 worker 必须返回统一 dossier 总哈希，并逐包确认全部已选证据包：核心范围 8 包、全量范围 11 包。
...
full 的交付摘要必须列出全部所选 stable master ID、本轮全部 8 或 11 个分析师，以及系统行情快照或
...
首次美国/台湾/市场数据检查不需要数据商密钥。可选的 DART、EDINET 数据源需要文档列出的
免费密钥，议会研究需要已认证的宿主/模型。下表明确列出这些边界。
...
大师读到的是**和分析师同一份已确立事实**（申报、行情、财务、宏观），分析师的证据包单独给出并标注为「其他席位的解读」而非事实。这个分离是关键：芒格看激励结构的地方分析师看的是毛利率，只有让他们各自取舍，这个议席才有存在意义。详见 [docs/attribution.md](../attribution.md)。
