...
> **Status: unreleased.** The feeds, the authoring mechanism and the voice layer are merged and
> verified. The authored method set is still empty, so seats run the identity proxy and this
> document describes what 1.0.0 contains rather than what is tagged today. It is tagged when
> `data/persona-v3-authored-methods.v1.mjs` carries the seats and the regenerated tree is green.
