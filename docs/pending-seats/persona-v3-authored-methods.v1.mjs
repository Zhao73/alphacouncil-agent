...
export const personaV3AuthoredMethods = Object.freeze({
  ...
  master_graham: {
    ...
    tools: [
      {
        tool_id: "master_graham.market_capitalisation",
        operation: "multiply",
        inputs: [{ fact_id: "market.price" }, { fact_id: "capital_allocation.share_count" }],
        output_id: "valuation.market_capitalisation",
        value_kind: "monetary",
        unit: "currency_units",
        purpose:
          "Graham's tests are all price-relative, so the absolute balance-sheet and earnings facts need the price of the whole company as their denominator.",
      },
      {
        tool_id: "master_graham.normalised_earnings_yield",
        operation: "divide",
        inputs: [{ fact_id: "financial.free_cash_flow_5y" }, { output_id: "valuation.market_capitalisation" }],
        output_id: "valuation.normalised_earnings_yield",
        value_kind: "ratio",
        unit: "decimal",
        purpose:
          "The earnings yield Graham compares against bonds, built from a five-year average so that no single good or bad year can carry the verdict.",
      },
      {
        tool_id: "master_graham.bond_yield_hurdle",
        operation: "multiply",
        inputs: [{ fact_id: "macro.aaa_corporate_yield" }, { literal: 2 }],
        output_id: "valuation.graham_bond_yield_hurdle",
        value_kind: "ratio",
        unit: "decimal",
        purpose:
          "Twice the high-grade corporate bond yield, so Graham's simplified criterion becomes a single comparison rather than arithmetic smuggled into a condition.",
      },
      {
        tool_id: "master_graham.net_current_asset_coverage",
        operation: "divide",
        inputs: [{ fact_id: "financial.net_current_asset_value" }, { output_id: "valuation.market_capitalisation" }],
        output_id: "valuation.net_current_asset_coverage",
        value_kind: "ratio",
        unit: "decimal",
        purpose:
          "Net current assets per unit of market price. Graham's net-net floor is a price at or below two thirds of that figure, which is coverage of 1.5 or better.",
      },
    ],
    eligibility: {
      all: [
    ...
    scoring: [
      {
        ...
        condition: { op: "gte", left: { output_id: "valuation.normalised_earnings_yield" }, right: { output_id: "valuation.graham_bond_yield_hurdle" } },
      },
      {
        ...
        coverage_weight: 1,
        rationale:
          "The Intelligent Investor ch. 14 defensive criterion: current price should not be more than fifteen times average earnings. Fifteen times is an earnings yield of one fifteenth, or 0.0667.",
        condition: { op: "gte", left: { output_id: "valuation.normalised_earnings_yield" }, right: { literal: 0.0667 } },
      },
      {
        rule_id: "graham_net_current_asset_bargain",
        points: 1,
        coverage_weight: 1,
        rationale:
          "The separate asset floor Graham describes in ch. 15 and in Security Analysis: buy at no more than two thirds of net current asset value, so working capital alone covers the price with a third to spare. Coverage of 1.5 is that same test written as a ratio.",
        condition: { op: "gte", left: { output_id: "valuation.net_current_asset_coverage" }, right: { literal: 1.5 } },
      },
    ],
    bands: [
      { min_ratio: 0, common_stance: "opposed", native_state: "reject" },
