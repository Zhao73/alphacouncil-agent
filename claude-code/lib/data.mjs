...
async function secTickers() {
  if (!secTickerMap) {
    const raw = await fetchJson("https://www.sec.gov/files/company_tickers.json", { timeoutMs: 20000 });
...
export async function getFundamentals(symbol, { instrument = null, price = null, currency = "USD" } = {}) {
  ...
  const doc = await fetchJson(url, { timeoutMs: 25000 });
  return {
    symbol,
    available: true,
    cik: inst.cik,
...
export async function getFilings(symbol, { forms = null, limit = 15, instrument = null } = {}) {
  ...
  const doc = await fetchJson(url, { timeoutMs: 20000 });
...
export async function getNews(query, { days: windowDays = 30, limit = 20 } = {}) {
  ...
  const all = parseRss(await fetchText(url));
...
export async function getMacro() {
  ...
  await Promise.all(Object.entries(FRED_SERIES).map(async ([id, label]) => {
    ...
    try {
      const rows = parseFredCsv(await fetchText(url, { timeoutMs: 20000 }));
