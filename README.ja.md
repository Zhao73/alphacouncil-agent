...
| ディベートの深さ | 単一パス bull → bear → PM | 3 ラウンド(主張/反論/Q&A)、各ラウンドで強気・弱気を並列 *(本版の設計)* |
| 主張の検証 | source ID の欠落を記録のみ | 主張ごとの敵対的検証:引用 URL の再取得 + 独立再導出 + 反証 *(本版の設計)* |
| モデルとコスト | 単一モデル | **役割ごとに選択** — 取証は Sonnet、ディベート/判定は Opus 4.8(全 Opus / 全 Sonnet も可) |
| 言語 | ユーザーの言語 | 全サブエージェント + ライブ workflow を通じてユーザーの言語 |

**正直なスコープ:** 同じモデルファミリー・同じプロンプト・同じ監査契約 —— 強みは文脈の分離、常時並列ファンアウト、決定的バリア、強制的な引用検証であり、より賢いモデルではありません。3 ラウンドのディベートと検証ステージは Claude Code 版の設計で、現在の共有コードは単一パスです。ライブ Web の鮮度とペイウォールは両版に等しく当てはまります。
