...
isolateHome();
const { createApiBackend, estimateCost } = await import("../src/backends/api.mjs");
const { createClaudeBackend, claudeArgs } = await import("../src/backends/claude.mjs");
const { selectBackend } = await import("../src/backends/index.mjs");
const { DESK_SCHEMA } = await import("../src/core/schemas.mjs");
...
test("engine selection: api with a key, claude without, clear error with neither", async () => {
  ...
  assert.equal((await selectBackend({ env: { ALPHA_CLAUDE_BIN: fakeClaude } })).name, "claude");
  await assert.rejects(selectBackend({ env: { ALPHA_CLAUDE_BIN: "/nonexistent/claude" } }), /No engine available/);
