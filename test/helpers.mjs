// Fixtures: an isolated ALPHA_HOME, a snapshot and a scripted backend.
...
export function isolateHome() {
  const dir = mkdtempSync(join(tmpdir(), "alpha-test-"));
  process.env.ALPHA_HOME = dir;
  process.env.ALPHA_NO_CACHE = "1";
  return dir;
}

export async function fakeSnapshot(symbol = "TEST") {
  const { evaluateLenses, LENS_IDS } = await import("../src/core/lenses.mjs");
  const s = {
    symbol,
...
export function fakeBackend({ fail = [], delayMs = 5 } = {}) {
  ...
  return {
    ...
    async call(req) {
      ...
      if (req.tier === "decision") return { data: decisionPacket(), costUsd: 0.2 };
