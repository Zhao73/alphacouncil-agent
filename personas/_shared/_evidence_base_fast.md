...
只返回符合原生 schema 的一个 JSON 对象，不要 Markdown、草稿或第二个对象。字段名、ticker、URL、source ID 与 enum 保持英文；面向读者的文本使用 {{language}}。
...
Return one JSON object matching the native schema: no markdown, draft, or second object. Keep keys, tickers, URLs, source IDs, and enums in English; write reader prose in {{language}}.
