  ...
  return {
    ...
    how_to_ask: copy({
      en: [
        ...
        "Requested or compatible remembered choices prefill the configuration. Require a fresh submission for this run; never reuse a receipt.",
        "Native selection is optional. To change methods, show the stable numbered table and accept numbers, stable IDs, or all in full mode.",
      ...
      zh: [
        "先展示本次配置，完整编号目录可展开。完整模式可单选、多选或全选；快速模式选 1–4 种方法。",
        "点名的方法或兼容的上次选择可预填，但本次仍需提交确认，不能复用旧凭证。",
        "宿主原生多选可选。需要更换方法时展示固定编号表，接受编号、稳定 ID；完整模式也可选 all。",
        "选择提交本身就是确认。不要再追加第二个确认问题，但没有 selection receipt 就不能开始研究。",
      ],
      ja: [
        "最初に今回の設定を表示し、番号付きの全一覧は展開可能にする。fullは1つ以上または全選択、quickは1–4を選ぶ。",
        "依頼文でマスターが指定済みでも、事前選択として表示したうえで今回の選択送信を求める。以前の実行の選択を黙って再利用しない。",
        "ホストの複数選択は任意。変更時は固定番号表から番号、stable IDを受け付け、fullではallも選択できる。",
        "選択の送信自体を確定とする。二重確認は追加しないが、selection receiptなしで調査を開始しない。",
      ],
      ko: [
        "먼저 이번 설정을 표시하고 전체 번호 목록은 펼칠 수 있게 한다. full은 1개 이상 또는 전체, quick은 1–4개 방법을 선택한다.",
        "요청에 마스터가 이미 지정되어 있어도 사전 선택으로 표시한 뒤 이번 실행의 선택 제출을 받아야 한다. 이전 실행의 선택을 조용히 재사용하지 않는다.",
        "호스트 다중 선택은 선택 사항이다. 변경 시 고정 번호표에서 번호와 stable ID를 받고 full에서는 all도 허용한다.",
