...
export function writeReportQuality(run, markdown, handoffMarkdown) {
  ...
  const quality = handoff ? {
    ...
    company_dossier_coverage: dossier,
...
export function writeArtifactIndex(run, debate = {}) {
  ...
  const lines = [
    ...
    existsSync(artifacts.company_dossier_json)
      ? `- ${label.dossier}: ${artifacts.company_dossier_json}`
      : "",
...
function handoffMethodTail(run, copy) {
  ...
  return [
    `<!-- ${HANDOFF_METHOD_TAIL_MARKER} -->`,
    "<details>",
    `<summary>${copy.masters} — ${selected.length}</summary>`,
    "",
...
export function userResponseMarkdown(run, manager) {
  ...
  const markdown = [
    ...
    "",
    `## ${copy.conclusion}`,
    `- ${copy.statusLabel}: ${localizedDisplayValue(run.status, run.language)} · ${copy.rating}: ${localizedDisplayValue(decisionAvailable ? manager.rating : "unavailable", run.language)} · ${copy.confidence}: ${localizedDisplayValue(decisionAvailable ? (manager.confidence || "low") : "unavailable", run.language)}`,
    decisionAvailable ? clipAtBoundary(manager.verdict || manager.summary, 400) : copy.noDecisionFriendly,
    `- ${copy.price}: ${priceLine}`,
    `- ${copy.valuation}: ${decisionAvailable ? (clipAtBoundary(manager.valuation_range, 280) || copy.missing) : localizedDisplayValue("unavailable", run.language)}`,
    `- ${copy.position}: ${decisionAvailable ? (clipAtBoundary(manager.position, 280) || copy.missing) : localizedDisplayValue("unavailable", run.language)}`,
    ...
    "",
    "<details>",
    `<summary>${copy.analysts} · ${copy.key} · ${copy.files}</summary>`,
    "",
    `## ${copy.status}`,
    `- ${copy.statusLabel}: ${localizedDisplayValue(run.status, run.language)}`,
    `- ${copy.contract}: ${run.council_mode === "quick" ? "quick_v1" : "full_v2"}`,
    `- ${copy.scope}: ${run.council_mode === "quick" ? copy.quickScope : copy.fullScope(fullCouncilCeiling(run), (run.tasks || []).length)}`,
    ...
    `- ${copy.dossierCoverage}: ${dossierLine}`,
    "",
    `## ${copy.judgment}`,
