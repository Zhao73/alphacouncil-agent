...
export function masterVoicePrompt(masterId, run, frozenOpinion) {
  ...
  return [
    ...
    [
      ...
      "A frozen out_of_scope stance withholds the VOTE. It never withholds the VIEW, and it is never a reason to answer the reader with a declination. This seat was selected because a reader wants this company seen through this method, so deliver that reading in full: every one of the five fields carries the same figures, source IDs and substance a voting seat carries. Do not open any field with what you cannot do. `what_i_see` opens on the company itself -- its economics, its numbers, its structure, whatever the evidence carries -- read through this method's own priorities and vocabulary. `how_my_method_reads_it` runs the method as far as the available evidence allows and reaches the furthest defensible conclusion it supports, then names in ONE sentence the specific input that could not be computed and what it would have decided. Only `would_i_act` states, in one sentence and not as its opening clause, that no scored vote is cast this round and why, followed by the qualified read the method does support. Say plainly that the unscored part is observation rather than a position -- a method that guesses at its own missing inputs stops being a method -- and end with the concrete condition that reopens the vote. This applies to every instrument: for a fund or index read the classification, concentration, top holdings, weights and aggregate figures; for an operating company read the business, margins, cash generation, balance sheet and price structure.",
      "For a frozen out_of_scope stance, never phrase any field as this method buying, selling, adding, trimming, overweighting or underweighting. In `where_i_disagree`, challenge the analytical inference or evidence use rather than restating somebody else's trade as a first-person trade action.",
    ].join(" "),
    frozenOpinion?.stance === "out_of_scope"
      ? [
        "OUT-OF-SCOPE ACTION SAFETY: never couple this method's first-person subject (I, we, my/our method) to buying, selling, shorting, adding, trimming, overweighting, underweighting, or an equivalent action in the output language.",
        "Factual compounds such as short interest or sell-side consensus, and clearly attributed third-party recommendations, remain allowed because they are evidence rather than this seat's action. Do not turn a quotation or rebuttal into a first-person trade commitment.",
        "In `where_i_disagree`, prefer: 'I reject that directional inference because ...' or 'I find that action conclusion unsupported because ...'. Do not write 'I would not buy/sell/short ...', which still couples the method to a trade verb and fails the runtime gate.",
      ].join(" ")
      : "",
