...
let diskCache = !process.env.ALPHA_NO_CACHE;
...
export function userAgent() {
  const contact = process.env.ALPHA_SEC_CONTACT || "research@alphacouncil.invalid";
  return `AlphaCouncil/3 (equity research; ${contact})`;
}
...
async function request(url, { headers = {}, timeoutMs = 12000 } = {}) {
  if (process.env.ALPHA_OFFLINE) throw new HttpError(`offline (ALPHA_OFFLINE): ${host(url)}`);
  let last;
