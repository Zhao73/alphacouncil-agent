...
// Emits events for a UI and saves every artifact under ~/.alphacouncil/runs/<id>/.
...
export function createRun({ symbol, mode = "deep", language = "en", question = "", engine = "host", models = {} }) {
  ...
  const run = {
    ...
    as_of: now.toISOString().slice(0, 10), created_at: now.toISOString(), engine, models, state: "running",
...
export async function research({ symbol, mode = "deep", language = "en", question = "", backend, onEvent = () => {}, signal, snapshot = null }) {
  ...
  try {
    ...
    try {
      ...
      let chars = 0;
      const r = await attempt(() => backend.call({ tier: "decision", system: p.system, user: p.user, schema: DECISION_SCHEMA, effort: "medium", timeoutMs: DECISION_TIMEOUT, signal: g.signal, onText: (d) => { chars += d.length; emit({ type: "activity", task: "decision", text: `writing… ${chars} chars` }); } }));
      spend(r.costUsd);
