...
export function guiDirectory(metaUrl = import.meta.url) {
  return dirname(fileURLToPath(metaUrl));
}
const HTML = readFileSync(join(guiDirectory(), "index.html"));
...
export function isAllowedHost(value) {
  ...
  const match = /^(localhost|127\.0\.0\.1|\[::1\])(?::(\d{1,5}))?$/iu.exec(value);
